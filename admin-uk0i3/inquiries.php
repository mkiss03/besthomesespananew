                            ...
                            <tr style="<?= $inq['status'] === 'new' ? 'background: #fff3cd;' : '' ?>">
                                ...
                                <td>
                                    <div class="message-preview">
                                        <?= e(substr($inq['message'], 0, 50)) ?><?= strlen($inq['message']) > 50 ? '...' : '' ?>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <button onclick="viewPropertyInquiryDetails(<?= $inq['id'] ?>)" class="btn btn-primary btn-view-details">
...
// Property inquiries and contact inquiries data
const propertyInquiries = <?= json_encode($inquiries) ?>;
const contactInquiries = <?= json_encode($contactInquiries) ?>;

// View property inquiry details
function viewPropertyInquiryDetails(id) {
    const inquiry = propertyInquiries.find(i => i.id == id);
    if (!inquiry) return;
    ...
    modalBody.innerHTML = `
        ...
        <div class="detail-row">
            <div class="detail-label">Érdeklődik:</div>
            <div class="detail-value">
                <a href="/admin-uk0i3/property-edit.php?id=${inquiry.property_id}" target="_blank">
                    <i class="fas fa-external-link-alt"></i> ${escapeHtml(inquiry.property_title)}
                </a>
            </div>
        </div>
        ` : ''}
